...
export default function ProjectChartPreview({ chartType, title, badge }: ProjectChartPreviewProps) {
  return (
    <div className="relative w-full h-48 sm:h-52 bg-slate-950/80 rounded-xl overflow-hidden border border-white/10 p-4 flex flex-col justify-between group-hover:border-[#4F8CFF]/40 transition-colors">
      
      {/* Top Bar of Graphic Mockup */}
      <div className="flex items-center justify-between border-b border-white/10 pb-2">
        <div className="flex items-center gap-1.5">
          <div className="w-2.5 h-2.5 rounded-full bg-red-500/80" />
          <div className="w-2.5 h-2.5 rounded-full bg-yellow-500/80" />
          <div className="w-2.5 h-2.5 rounded-full bg-emerald-500/80" />
          <span className="ml-2 text-[10px] font-mono text-slate-400 truncate max-w-[150px]">{title}</span>
        </div>
      ...
      {/* SVG Analytical Render based on chartType */}
        ...
        {chartType === 'pie' || chartType === 'donut' ? (
          <svg className="w-full h-24" viewBox="0 0 200 100">
            ...
            <g transform="translate(130, 25)">
              ...
              <text x="12" y="8" fill="#cbd5e1" fontSize="9" fontFamily="monospace">Core (60%)</text>
              <rect x="0" y="16" width="8" height="8" rx="2" fill="#8B5CF6" />
              <text x="12" y="24" fill="#cbd5e1" fontSize="9" fontFamily="monospace">Growth (25%)</text>
              <rect x="0" y="32" width="8" height="8" rx="2" fill="#00E5A8" />
              <text x="12" y="40" fill="#cbd5e1" fontSize="9" fontFamily="monospace">Risk (15%)</text>
            </g>
          </svg>
        ) : null}
      ...
      <div className="flex items-center justify-between text-[10px] font-mono text-slate-400 pt-1 border-t border-white/5">
        <span className="text-[#00E5A8]">● LIVE MODEL</span>
        <span>DAX / SQL ETL</span>
      </div>
    </div>
  );
}
